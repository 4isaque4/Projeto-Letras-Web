...
function timelineVisual(event: HistoryItem): {
...
export default function AlfabetizandoDetalhe() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        setDetail(response);
        void apiGet(
          `/painel/fotos-atividade?studentId=${encodeURIComponent(id)}`,
        )
          .then((items) => {
            if (active)
              setActivitySubmissions(
                Array.isArray(items) ? (items as ActivitySubmission[]) : [],
              );
          })
          .catch(() => {
            if (active) setActivitySubmissions([]);
          });
      } catch (fetchError) {
        if (!active) {
          return;
        ...
        setError(
          fetchError instanceof Error
            ? fetchError.message
            : "Falha ao carregar detalhe do alfabetizando.",
        );
      } finally {
        if (active) {
          setLoading(false);
  ...
  const runHistoryAction = async (history: HistoryItem) => {
    ...
    const action =
      history.queueType === "progresso" ? "desbloquear" : "resolver";
    ...
    try {
      ...
      await apiPatch(`/painel/fila/${history.id}`, {
        ...
        responseMessage:
          action === "resolver" ? "Pode continuar a atividade" : undefined,
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <button
          onClick={() => navigate(-1)}
          className="p-2 border border-gray-400 hover:bg-gray-100"
        >
          <ArrowLeft className="w-5 h-5" />
        </button>
        <div className="flex-1">
          <h1 className="text-2xl font-bold text-gray-900">
            Detalhe do Alfabetizando
          </h1>
          <p className="text-sm text-gray-600 mt-1">
            Acompanhamento completo do progresso e histórico
          </p>
        </div>
        ...
        <>
          <div className="border border-gray-300 bg-white p-6">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <div>
                ...
                <span className="px-2 py-1 text-xs border border-gray-400 inline-block">
                  {statusLabel}
                </span>
              </div>
            </div>
          ...
          <div className="border border-gray-300 bg-white">
            ...
            {detail.progresso.length === 0 ? (
              <StateDisplay
                type="empty"
                message="Sem progresso registrado para este alfabetizando."
              />
            ) : (
          ...
          <div className="border border-gray-300 bg-white">
            ...
            {detail.tentativas.length === 0 ? (
              <StateDisplay
                type="empty"
                message="Sem tentativas registradas."
              />
            ) : (
          ...
          <div className="border border-gray-300 bg-white">
            <div className="p-4 border-b border-gray-300">
              <h3 className="font-bold text-gray-900">
                Submissões (Fotos/Áudios)
              </h3>
            </div>
            <ActivitySubmissionsGallery items={activitySubmissions} />
          </div>
          ...
          <div className="border border-gray-300 bg-white">
            <div className="p-4 border-b border-gray-300 flex items-center justify-between">
              ...
              <span className="text-xs text-gray-500">
                {detail.historico.length} eventos
              </span>
            </div>
            {detail.historico.length === 0 ? (
              <StateDisplay
                type="empty"
                message="Sem eventos para este alfabetizando."
              />
            ) : (
              <ol
              ...
              >
                {detail.historico.map((event, index) => {
                  ...
                  const prevDay =
                    index > 0
                      ? dayBucket(detail.historico[index - 1].data)
                      : "";
                  ...
                  return (
                    <li key={event.id} className="relative">
                      ...
                      <div className="flex items-start gap-3">
                        <div
                          className={`w-8 h-8 flex items-center justify-center border ${tone} shrink-0`}
                        >
