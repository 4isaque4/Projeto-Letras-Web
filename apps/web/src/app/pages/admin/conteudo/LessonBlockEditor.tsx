      ...
      return {
        ...
        rows: block.rows.map((row) => ({
          ...
          options: row.optionsText
            ...
            .filter(Boolean),
          correctOptions: [
            row.correctOption.trim().toUpperCase().slice(0, 1),
          ].filter(Boolean),
...
export function deserializeToBlocks(raw: string): LessonBlock[] {
  ...
  try {
    ...
    return [
      { id: genId("text"), type: "text", content: raw, audience: "educator" },
    ];
  }
}
...
function convertV1ToBlocks(parsed: Record<string, unknown>): LessonBlock[] {
  ...
  const eg =
    typeof parsed.educatorGuidance === "string"
      ? parsed.educatorGuidance.trim()
      : "";
  const ls =
  ...
  const items = Array.isArray(ex.items)
    ? (ex.items as Record<string, unknown>[])
    : [];
...
function makeDefaultBlock(type: LessonBlock["type"]): LessonBlock {
  switch (type) {
    ...
    case "gif":
      return { id: genId("gif"), type: "gif", url: "", caption: "" };
    case "video":
...
> = {
  ...
  "exercise-match-letter": {
    ...
    label: "Exercício: Marcar Letra",
    color: "border-amber-200 bg-amber-50",
  },
  "exercise-mark-images": {
    icon: <Target className="h-3.5 w-3.5" />,
    label: "Exercício: Marcar Imagens",
...
function BlockWrapper({
  ...
  return (
    <div
      className={`rounded border ${meta.color} overflow-hidden ${selected ? "ring-2 ring-slate-700 ring-offset-1" : ""}`}
    >
      {/* Header */}
      <div
        className={`flex items-center justify-between px-3 py-2 ${selected ? "border-b border-inherit" : ""}`}
      >
        <button
          type="button"
          onClick={onSelect}
          className="flex flex-1 items-center gap-2 text-left text-xs font-semibold text-slate-600"
        >
          <GripVertical className="h-3.5 w-3.5 text-slate-300" />
          {meta.icon}
          <span>{meta.label}</span>
          <span className="font-normal text-slate-400">
            {selected ? "Editando" : "Selecionar para editar"}
          </span>
        </button>
      ...
      {/* Body */}
      {selected ? <div className="p-3">{children}</div> : null}
    </div>
  );
}
...
function TextBlockEditor({
  ...
  const AUDIENCE_OPTIONS: {
    ...
    {
      value: "educator",
      label: "Para o alfabetizador",
      icon: <User className="h-3.5 w-3.5" />,
    },
    {
      value: "learner",
      label: "Para o alfabetizando",
  ...
  const handleGenerateAudio = async () => {
    ...
    setTtsError(null);
    try {
      const data = (await apiPost("/painel/tts/generate", { text })) as {
        url?: string;
      };
      ...
      {showTts && (
        <div className="space-y-1.5 border-t border-slate-100 pt-2">
          ...
          <p className="text-xs text-slate-400">
            O áudio gerado substitui o texto na tela do alfabetizando — ele ouve
            em vez de ler.
          </p>
        </div>
      )}
...
function MediaBlockEditor({
  ...
  block: ImageBlock | GifBlock | VideoBlock | AudioBlock;
  onChange: (
    updates: Partial<ImageBlock | GifBlock | VideoBlock | AudioBlock>,
  ) => void;
}) {
  const placeholder =
    block.type === "image" || block.type === "gif"
      ? `URL ${block.type === "gif" ? "do GIF" : "da imagem"} (https://...)`
      : block.type === "video"
        ? "URL do vídeo (https://...)"
        : "URL do áudio (https://...)";
...
function ExerciseMarkImagesEditor({
  ...
  const updateRow = (id: string, updates: Partial<MarkImageRow>) => {
    onChange({
      rows: block.rows.map((r) => (r.id === id ? { ...r, ...updates } : r)),
    });
  };
  ...
  const applyBulk = () => {
    ...
    const rows: MarkImageRow[] = lines.map((line) => {
      const [labelRaw = "", correctRaw = "", imageRaw = "", audioRaw = ""] =
        line.split("|").map((t) => t.trim());
      const isCorrect = ["sim", "s", "1", "true", "x", "yes"].includes(
        correctRaw.toLowerCase(),
      );
      return {
        id: genId("row"),
        label: labelRaw || "Item",
    ...
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) =>
              onChange({
                expectedSelections: Math.max(1, Number(e.target.value)),
              })
            }
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) =>
              onChange({ maxAttempts: Math.max(1, Number(e.target.value)) })
            }
            className="mt-0.5 w-full border border-slate-300 bg-white px-2 py-1.5 text-sm focus:outline-none"
          />
        </div>
          ...
          <div className="mt-2 space-y-2 rounded border border-slate-200 bg-white p-3">
            <p className="text-[11px] text-slate-500">
              ...
              <code className="rounded bg-slate-100 px-1">
                nome | sim/não | imageUrl | audioUrl
              </code>
            </p>
            <textarea
              rows={5}
      ...
      <div className="space-y-2">
        {block.rows.map((row, idx) => (
          <div
            key={row.id}
            className="grid grid-cols-[auto_1fr_auto] gap-2 rounded border border-slate-200 bg-white p-2.5"
          >
            {/* Correct toggle */}
            <button
              type="button"
              onClick={() =>
                updateRow(row.id, { isCorrectTarget: !row.isCorrectTarget })
              }
              title={
                row.isCorrectTarget
                  ? "Correto — clique para marcar como errado"
                  : "Errado — clique para marcar como correto"
              }
            ...
            <div className="grid grid-cols-1 gap-1.5 sm:grid-cols-2">
              ...
              <input
                value={row.audioUrl}
                onChange={(e) =>
                  updateRow(row.id, { audioUrl: e.target.value })
                }
    ...
    <div className="space-y-3">
        ...
        {showReinforcement && (
          <div className="mt-2 grid grid-cols-1 gap-2 rounded border border-slate-200 bg-white p-3 sm:grid-cols-2">
            ...
            <input
              ...
              onChange={(e) =>
                onChange({ reinforcementAudioUrl: e.target.value })
              }
        ...
        {showBulk && (
          <div className="mt-2 space-y-2 rounded border border-slate-200 bg-white p-3">
            <p className="text-[11px] text-slate-500">
              Formato:{" "}
              <code className="rounded bg-slate-100 px-1">
                palavra | opções | correta | imageUrl | audioPalavra |
                audioSoletracao
              </code>
            </p>
            <textarea
      ...
      <div className="space-y-2">
        {block.rows.map((row, idx) => (
          <div
            key={row.id}
            className="space-y-1.5 rounded border border-slate-200 bg-white p-3"
          >
            <div className="flex items-center gap-2">
              ...
              <input
                ...
                onChange={(e) =>
                  updateRow(row.id, {
                    correctOption: e.target.value.toUpperCase(),
                  })
                }
                placeholder="✓"
            ...
            <div className="grid grid-cols-1 gap-1.5 pl-7 sm:grid-cols-3">
              ...
              <input
                ...
                onChange={(e) =>
                  updateRow(row.id, { wordAudioUrl: e.target.value })
                }
              ...
              <input
                ...
                onChange={(e) =>
                  updateRow(row.id, { spellingAudioUrl: e.target.value })
                }
...
function BlockBody({
  ...
  if (block.type === "exercise-mark-images") {
    return (
      <ExerciseMarkImagesEditor block={block} onChange={(u) => onChange(u)} />
    );
  }
  if (block.type === "exercise-match-letter") {
    return (
      <ExerciseMatchLetterEditor block={block} onChange={(u) => onChange(u)} />
    );
  }
  return null;
}
...
export default function LessonBlockEditor({
  ...
  const update = (id: string, updates: Partial<LessonBlock>) => {
    onChange(
      blocks.map((b) =>
        b.id === id ? ({ ...b, ...updates } as LessonBlock) : b,
      ),
    );
  };
