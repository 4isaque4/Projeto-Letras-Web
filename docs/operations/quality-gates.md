# Rotina de qualidade - Projeto Letras Web

## Comandos locais

- `pnpm quality`: valida ambiente de producao e copy de produto.
- `pnpm test`: roda testes de contrato da API.
- `pnpm build`: compila o painel web.
...
- Tema continua sendo universo de interesse, nao estrutura didatica.
- Copy visivel usa "Aulas e Midias", nunca "CMS".
- Escritas relevantes registram `sync_events`.
- Mocks seguem desligados em producao.
- Migrations SQL, quando necessarias, estao em `infra/supabase/migrations/`.
- Contrato realtime v1 continua tolerante a eventos desconhecidos.
